...
1. The three master passwords you enter are combined.
2. The combined string is hashed using **SHA-256**.
3. That hash is encoded into a URL-safe base64 string to create a valid **Fernet Key**.
4. This key is used to decrypt the vault file upon login and re-encrypt it when saving new entries.
...
1. Download the latest `.zip` release from the **Releases** tab and extract it.
2. Place the executable in a dedicated folder (e.g., `Documents/PasswordVault`). *Note: The encrypted vault file (`my_vault.enc`) will be generated in the same folder as the executable.*
3. Double-click to run. 
*(Note: Because this is an unsigned, locally-built executable, Windows Defender may flag it. You can safely click "More info" -> "Run anyway" to use the vault.)*
...
1. Clone the repository:
   ```bash
   git clone [https://github.com/f4209138-ctrl/Password-Generator-Vault.git](https://github.com/f4209138-ctrl/Password-Generator-Vault.git)
...
Run the script:
Here are the updated versions of your README and Landing Page, now properly linked to your specific repository!

1. Updated README.md
The only change here is the git clone URL in the "Running from Source" section.
